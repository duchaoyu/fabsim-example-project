...
sec = ax.secondary_xaxis('top', functions=(lambda x: x/E2*1e3, lambda w: w*E2/1e3))
sec.set_xlabel('equivalent width of same-stiffness fabric (mm)', fontsize=8.5, labelpad=2)
...
ax.text(0, 0.42,
        'Non-sliding cable on the region boundary of\nstrategy E, rest length read off the pre-strained\n'
        'rest shape V0_mod, so the seam is loaded at\nnominal (8% stretched, 72/76 segments taut).\n\n'
        'Saturation is the rigid-seam bound: no model of\nthe transition that only adds axial stiffness\n'
        'along the line can exceed it.',
        fontsize=8.5, va='top', transform=ax.transAxes, color='0.25')
