...
def fd_inflate(mesh, pressure):
    ...
    for _ in range(5):
        ...
        load = - 240 * 0.005 * 0.1 # 1500kg/m3 * 0.02 = 30kg/m2
...
folder = os.path.abspath("/Users/duch/Documents/Github/phd/pneu_fofin/data/crossvault")
# file_name = "shell_four_corners.obj"
# file_path = os.path.join(folder, file_name)
file_network = os.path.join(folder, "cross_vault_smooth.json")
file_session = os.path.join(folder, "mesh_out_cross_20240404125451.json")
mesh_ref = Mesh.from_json(file_network) # network 
mesh_0 = Mesh.from_json(file_session) # network 
# mesh_0.flip_cycles()
anchors = list(mesh_0.vertices_where({'is_anchor': True}))

file_ref = os.path.join(folder, "cross_vault_smooth.json")
...
# mesh_0.update_default_edge_attributes(qpre=8.0)
...
FOLDER = os.path.join(HERE, 'data', 'crossvault')
print(formatted_time)
mesh_1.to_json(os.path.join(FOLDER, 'mesh_out_cross_{}.json'.format(formatted_time)))
