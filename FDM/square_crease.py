...
def fd_inflate(mesh, pressure):
    ...
    for _ in range(5):
        ...
        load = - 30 # 1500kg/m3 * 0.02 = 30kg/m2
...
folder = os.path.abspath("/Users/duch/Documents/PhD/knit/2024_prototypes/rectangle/exp/")
file_network = os.path.join(folder, "gentle_unit_m.json")
mesh_ref = Mesh.from_json(file_network) # network 
mesh_0 = Mesh.from_json(os.path.join(folder, "session","mesh_out_20250109173008.json")) # reference mesh
# mesh_0 = Mesh.from_json(os.path.join(folder, "gentle_unit_m.json")) # reference mesh
...
for vkey in mesh_ref.vertices():
    if mesh_ref.is_vertex_on_boundary(vkey):
        anchors.append(vkey)
# anchors = list(file_network.vertices_where({'is_anchor': True}))

file_ref = os.path.join(folder, "gentle_unit_m.json")
...
# mesh_0.update_default_edge_attributes(qpre=15)
# sel_edges = [(1, 171), (34, 235), (34, 338), (53, 363), (53, 434), (74, 149), (74, 237), (149, 431), (171, 434), (202, 228), (202, 334), (223, 256), (228, 338), (235, 250), (237, 266), (250, 363), (256, 431), (266, 414), (334, 414)]
# for (a,b) in mesh_0.edges():
#     # print(a,b)
#     if (a,b) in sel_edges or (b,a) in sel_edges:
#         # print(a,b)
#         mesh_0.edge_attribute((a,b), 'qpre', 300)

# for edge in mesh_0.edges():
#     print(mesh_0.edge_attribute(edge, 'qpre'))
...
for _ in range(steps):
    ...
    for i, edge in enumerate(mesh_1.edges()):
        q = qpre[i]
        if q < 0.01: # q should not be negative, constraint 
            q = 0.01
        mesh_1.edge_attribute(edge, 'qpre', q)    
    fd_inflate(mesh_1, pressure=pressure)
    # print(mesh_1.edges_attribute('qpre'))
    ...
    if _ % 500 == 0 and _ != 0:
        ...
        mesh_1.to_json(os.path.join(folder, "session", 'mesh_out_{}.json'.format(formatted_time)))
...
viewer.add(mesh_ref, color=(0, 1, 0)) # green
viewer.show()
