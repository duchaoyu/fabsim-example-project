...
file_network = os.path.join(folder, "cross_vault_smooth.obj") # OBJ
mesh_0 = Mesh.from_obj(file_network) # network 

# mesh_0.flip_cycles()
# anchors = list(mesh_0.vertices_where({'is_anchor': True}))

file_ref =  os.path.join("/Users/duch/Documents/Github/phd/pneu_fofin/data", "crossvault", "cross_vault_smooth.json")
...
mesh_0.update_default_edge_attributes(qpre=1.0)
...
loop = [146, 110, 153, 168, 94, 97, 70, 68, 156, 83, 82, 39, 40]
for (a, b) in zip(loop[:-1], loop[1:]):
    mesh_0.edge_attribute((a, b), 'qpre', 3.)
    
loop2 = [133, 123, 112, 14, 12, 17, 70, 69, 128, 7, 4, 10, 107]
for (a, b) in zip(loop2[:-1], loop2[1:]):
    mesh_0.edge_attribute((a, b), 'qpre', 3.)
...
mesh_1.to_json(os.path.join(FOLDER, 'mesh_out_corss_{}.json'.format(formatted_time)))
