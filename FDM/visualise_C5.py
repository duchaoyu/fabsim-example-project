"""
Visualise FDM force flow in C5 result mesh.
...
Saves data/C5/C5_force_flow.png
"""
import os, glob
import numpy as np
import matplotlib
...
# Pick the most recent C5 FDM result
candidates  = sorted(glob.glob(os.path.join(HERE, "data", "mesh_out_C5_*.json")))
if not candidates:
    raise SystemExit("No mesh_out_C5_*.json found in data/ — run fofin_C5.py first.")
RESULT_JSON = candidates[-1]
TARGET_OBJ  = os.path.join(HERE, "input", "C5.obj")
OUT_DIR     = os.path.join(HERE, "data", "C5")
OUT_PNG     = os.path.join(OUT_DIR, "C5_force_flow.png")
os.makedirs(OUT_DIR, exist_ok=True)

print(f"Loading FDM result: {os.path.basename(RESULT_JSON)}")
...
# ── Geometry extents (for axes scaling) ───────────────────────────────────────
...
ax1.set_title("3D: axial force  f = q·l\n(green = target C5)", color="white", fontsize=10)
...
for pane in (ax3.xaxis.pane, ax3.yaxis.pane, ax3.zaxis.pane):
    pane.fill = False
pad = 0.5
ax3.set_xlim(x_min - pad, x_max + pad)
ax3.set_ylim(y_min - pad, y_max + pad)
ax3.set_zlim(0, max(z_max + pad, 1.0))
...
ax4.text(x_min + 1, -0.3, "slice x≈0  (y-z)", color="cyan",   fontsize=8)
ax4.text(x_off + 1, -0.3, "slice y≈0  (x-z)", color="orange", fontsize=8)
