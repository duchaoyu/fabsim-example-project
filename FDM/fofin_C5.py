"""
FDM form-finding for C5.obj target geometry.
...
Pipeline:
  1. Load C5.obj as target surface S
...
INPUT   = os.path.join(HERE, "input", "C5.obj")
...
# ── Optimise ──────────────────────────────────────────────────────────────────
print(f"C5 FDM optimisation: {len(free)} free nodes, {n_e} edges")
span_x = max(p[0] for p in target_xyz.values()) - min(p[0] for p in target_xyz.values())
height = max(p[2] for p in target_xyz.values())
print(f"Target span: {span_x:.2f} m, height: {height:.2f} m  |  pressure={PRESSURE}, q_init={Q_INIT}")
...
print(f"Final RMSE to target: {rmse:.4f} m  (span {span_x:.2f} m → {100*rmse/span_x:.2f}%)")

# ── Save ──────────────────────────────────────────────────────────────────────
ts   = datetime.datetime.now().strftime("%Y%m%d%H%M%S")
out  = os.path.join(DATA, f"mesh_out_C5_{ts}.json")
...
try:
    ...
    viewer.add(mesh_target, color=(0.3, 0.8, 0.3))   # green = target (C5)
