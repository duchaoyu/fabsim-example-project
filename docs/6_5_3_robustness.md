...
The boundary radius and the cable rest length are set as lengths, not as fractions, so they are plotted against the top axis in millimetres. At the ±2 mm of 5.5.2 the boundary contributes about 1.9 mm of surface deviation on both geometries. Ten millimetres of radius error would give 7.5 mm and ten of cable error 2.9 mm — the cable is the gentler of the two per millimetre, and only overtakes the stretch factors when expressed as a percentage because it is 1.32 m long. Both are quantities a workshop can measure directly, which makes them the two tolerances in this study that could be replaced by a measurement tomorrow.
