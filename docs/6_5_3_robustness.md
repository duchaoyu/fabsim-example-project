...
*Figure 6.31: Deviation from the designed equilibrium against how wrong each parameter is, 0 to 2% of nominal for the material and pre-strain parameters in (a) and (b), and 0 to 20 mm for the two that are set as lengths in (c) and (d) — a boundary ring is anchored to a distance and a cable is cut and taken up by one, so a percentage of nominal is a detour through a number nobody sets. The dotted line in (c) and (d) is the ±2 mm of 5.5.2. Curves are the worse of the two signs at each magnitude, and are linear throughout, so a response may be read at whatever tolerance the measurements eventually give. 180 solves across the two geometries.*
