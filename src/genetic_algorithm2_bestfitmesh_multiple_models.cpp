...
// List of pressures
std::vector<double> pressures = {1000, 1100, 1200, 1000, 1100, 1200};
std::vector<std::vector<Eigen::Vector3d>> face_vectors_list;
...
fsim::Mat3<double> solve(double modulus1,
                         double modulus2,
...
struct Individual {
    ...
    std::vector<fsim::Mat3<double>> Vsolves;
    ...
    Individual(double mod1_min, double mod1_max, double mod2_min, double mod2_max,
               ...
               const std::vector<std::vector<Eigen::Vector3d>>& face_vectors_list)
               {
        ...
        stretch_factor = sf_min + (std::rand() / (double)RAND_MAX) * (sf_max - sf_min);
         updateFitness(pressures, V_refs, face_vectors_list);
...
//        fitness = objectiveFunction(Vsolve, V_ref);
    ...
    void updateFitness(const std::vector<double>& pressures,
                       const std::vector<fsim::Mat3<double>>& V_refs,
                       const std::vector<std::vector<Eigen::Vector3d>>& face_vectors_list) {
//        Vsolve = solve(modulus1, modulus2, poisson_ratio, stretch_factor, pressure, face_vectors);
//        fitness = objectiveFunction(Vsolve, V_ref);
        ...
        for (size_t i = 0; i < pressures.size(); ++i) {
            ...
            const std::vector<Eigen::Vector3d>& face_vectors = face_vectors_list[i];
...
double modulus1_min = 3000; // Example values
double modulus1_max = 20000;

// Define bounds for modulus2
double modulus2_min = 3000;
...
Individual crossover(const Individual& parent1,
  ...
  Individual child = parent1; // Start with parent1's genes
    ...
    child.updateFitness(pressures, V_refs, face_vectors_list);
    return child;
}
...
void mutate(Individual& individual,
    ...
    individual.updateFitness(pressures, V_refs, face_vectors_list);
}
...
void geneticAlgorithm(const std::vector<double>& pressures,
  ...
  for (int i = 0; i < populationSize; ++i) {
    population.push_back(Individual(modulus1_min, modulus1_max,
                                        ...
                                        pressures, V_refs, face_vectors_list));
  }
  ...
  for (int generation = 0; generation < maxGenerations; ++generation) {
    ...
    Individual child1 = crossover(parent1, parent2, pressures, V_refs, face_vectors_list);
    Individual child2 = crossover(parent2, parent1, pressures, V_refs, face_vectors_list);

    // Mutation
    mutate(child1, pressures, V_refs, face_vectors_list);
    mutate(child2, pressures, V_refs, face_vectors_list);
...
int main() {
  ...
  std::vector<std::string> ref_mesh_filenames = {
    "/Users/duch/Documents/PhD/knit/2024_prototypes/callibration/flat_changing_directions/shortrow_p10.off",
    "/Users/duch/Documents/PhD/knit/2024_prototypes/callibration/flat_changing_directions/shortrow_p11.off",
    "/Users/duch/Documents/PhD/knit/2024_prototypes/callibration/flat_changing_directions/shortrow_p12.off",
...
//    std::cout << V_ref << std::endl;
  ...
  young_modulus1= 5917;  // knit: ~50kPa https://journals.sagepub.com/doi/pdf/10.1177/0040517510371864
  young_modulus2 = 16733;
  thickness = 1.0;
  poisson_ratio = 0.152;
  stretch_factor = 1.009;
...
for (size_t k = 0; k < pressures.size(); ++k) {
  std::vector<Eigen::Vector3d> face_vectors;
  ...
  face_vectors_list.push_back(face_vectors);
  }
  else{
    std::cout << k << std::endl;
...
for (int i = 0; i < populationSize; ++i) {
 ...
 geneticAlgorithm(pressures, V_refs, face_vectors_list);
