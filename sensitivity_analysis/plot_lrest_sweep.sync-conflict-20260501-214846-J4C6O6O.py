...
def plot_lrest_sweep(save=True):
    ...
    for col_idx, motif in enumerate(motifs):
        ...
        for case_key, row_data in ([("no_cable", nocable_row)]
            ...
            for fixed_axis, ls in [(0, "-"), (1, "--")]:
                pos_s, K_s = _gauss_curv_section(verts_d, fixed_axis)
                if len(pos_s) < 2: continue
                K_all.extend(K_s[np.isfinite(K_s)])
                ax4.plot(pos_s, K_s, color=color, ls=ls, lw=lw, alpha=alpha)

        if K_all:
            K_hi = np.percentile(K_all, 97)
            K_lo = np.percentile(K_all, 3)
            pad  = 0.15 * (K_hi - K_lo + 1e-9)
            ax4.set_ylim(K_lo - pad, K_hi + pad)

        ax4.axhline(0, color="0.8", lw=0.5, ls=":")
        ax4.set_xlabel("position along section  (mm)")
        ax4.set_ylabel(r"Gaussian curvature  $K$  (m$^{-2}$)")
        ax4.set_title(f"Motif {motif} — Gaussian curvature  (solid=x=0, dashed=y=0)")
